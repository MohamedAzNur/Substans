...
-- Videolektioner og reel afspilningsfremgang. Spring frem i videoen tæller ikke
-- som set; klienten gemmer kun de intervaller, eleven faktisk har afspillet.
alter table public.learning_items
...
create index if not exists video_watch_progress_item_idx on public.video_watch_progress(learning_item_id);
...
create policy "Members read relevant video progress" on public.video_watch_progress
for select to authenticated using (
  student_id in (select public.current_student_ids())
  or public.is_admin()
  or exists (
    select 1 from public.learning_items item
...
create policy "Students create own video progress" on public.video_watch_progress
...
create policy "Students update own video progress" on public.video_watch_progress
for update to authenticated
using (exists (select 1 from public.students student where student.id = video_watch_progress.student_id and student.student_profile_id = auth.uid()))
with check (
  exists (
    select 1 from public.students student
    join public.class_enrollments enrollment on enrollment.student_id = student.id
